...
<c:if test="${empty dataEmpty }">
	<c:forEach var="list" items="${Feeds}" varStatus="mainstatus" begin="${start }" end="${end }">
					...
					<div class="w3-center">
						...
						<c:if test="${list.feed_grade eq 'editor'}">
							...
							<img id="${list.feed_id}" class="showMask"
								...
								onclick="document.getElementById('popup').style.display='block'<%-- editorfeed('${fn:length(list.img_name)}','${list}','${list.feed_id}','${list.img_name}','${list.makecontent}','${list.replelist}','${fn:length(list.replelist)}','${list.food_id}','${list.hashtaglist}'); --%>">
							<br>
									${list.recipe_name }
									</c:if>
						<c:if test="${list.feed_grade eq 'nomal'}">
							<img id="${list.feed_id}" class="showMask"
								src="<%=request.getContextPath()%>/fileSave/${list.img_name[0]}"
								style="width: 100%; height: 200px"
								onclick="document.getElementById('popup').style.display='block'<%-- nomalfeed('${fn:length(list.img_name)}','${list}','${list.feed_id}','${list.img_name}','${list.makecontent}','${list.replelist}','${fn:length(list.replelist)}','${list.hashtaglist}'); --%>">
							<br>
									${list.recipe_name }
									</c:if>
					</div>
