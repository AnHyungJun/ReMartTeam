<%@ page language="java" contentType="text/html; charset=utf-8"
	pageEncoding="utf-8"%>

<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">

<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=EUC-KR">
<title>TV레시피</title>
</head>
<body>
	<div style="margin-top: 180px"></div>

	<div class="w3-row w3-center">
		<div class="w3-col" style="width: 20%">
			<p>&nbsp;</p>
		</div>
		<div class="w3-col" style="width: 60%">
			<div class="w3-center" style="font-size:12px;">
				<a class="w3-button w3-white w3-border w3-hover-white" href="#">냉장고를 부탁해</a> <a
					class="w3-button w3-white w3-border w3-hover-white" href="#">집밥 백선생</a> <a
					class="w3-button w3-white w3-border w3-hover-white" href="#">알토란</a>
			</div>
		</div>
		<div class="w3-col" style="width: 20%">
			<p>&nbsp;</p>
		</div>
	</div>
	<br>
	<br>
	<div>
		<table style="width: 60%; margin: auto;" class="w3-table w3-bordered">

			<tr class="w3-white" style="font-size: 12px;">
				<td align="center" width="50">번호</td>
				<td align="center" width="250">제목</td>
				<td align="center" width="150">작성일</td>
				<td align="center" width="50">조회</td>
			</tr>

			<tr height="40" class="w3-white" style="font-size: 12px;">
				<td align="center" width="50">5</td>
				<td align="center" width="250">김밥</td>
				<td align="center" width="150">2017/11/15</td>
				<td align="center" width="50">4</td>
			</tr>

			<tr height="40" class="w3-white" style="font-size: 12px;">
				<td align="center" width="50">4</td>
				<td align="center" width="250">불고기</td>
				<td align="center" width="150">2017/11/15</td>
				<td align="center" width="50">4</td>
			</tr>
			<tr height="40" class="w3-white" style="font-size: 12px;">
				<td align="center" width="50">3</td>
				<td align="center" width="250">김치찌개</td>
				<td align="center" width="150">2017/11/15</td>
				<td align="center" width="50">3</td>
			</tr>
			<tr height="40" class="w3-white" style="font-size: 12px;">
				<td align="center" width="50">2</td>
				<td align="center" width="250">샌드위치</td>
				<td align="center" width="150">2017/11/15</td>
				<td align="center" width="50">4</td>
			</tr>
			<tr height="40" class="w3-white" style="font-size: 12px;">
				<td align="center" width="50">1</td>
				<td align="center" width="250">제육볶음</td>
				<td align="center" width="150">2017/11/15</td>
				<td align="center" width="50">5</td>
			</tr>

		</table>
	</div>
</body>
</html>